...
def inicio_final(a, b):
  inicio_a = a[:len(a)//2 if len(a) %2==0 else len(a)//2+1]
  inicio_b = b[:len(b)//2 if len(b) %2==0 else len(b)//2 + 1]

  final_a = a[len(a)//2 if len(a) %2 == 0 else len(a)//2 + 1:]
  final_b = b[len(b)//2 if len(b) %2 == 0 else len(b)//2 + 1:]
